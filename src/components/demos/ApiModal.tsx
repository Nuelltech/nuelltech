...
import { X, Code, Terminal, Send, Link, Database, Sparkles } from 'lucide-react';
...
const apiScenarios: Record<ApiType, ApiScenario> = {
  erp: {
    ...
    method: 'POST',
    ptDesc: 'Sincronização de stock em tempo real entre o software de faturação/POS (Primavera, Sage ou PHC) e o inventário digital central.',
    enDesc: 'Real-time inventory synchronization between local POS/billing software (Primavera, Sage, or PHC ERP) and the central data hub.',
    requestPayload: {
      event: "inventory_updated",
      provider: "Primavera ERP v10",
    ...
    ptTimeline: [
      '[17:45:00] Pedido POST recebido da API Primavera ERP.',
    ...
    enTimeline: [
      '[17:45:00] POST request received from Primavera ERP API.',
...
export default function ApiModal({ isOpen, onClose, pt }: ApiModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md transition-opacity duration-300">
      ...
      {/* Modal */}
      <div className="bg-[#04060C] border border-brand-border/60 rounded-2xl w-full max-w-5xl h-[85vh] max-h-[750px] flex flex-col overflow-hidden shadow-2xl relative">
        
        {/* Header */}
        <div className="flex justify-between items-center px-6 py-4 border-b border-brand-border/60 bg-[#090D1A]">
          <div className="flex items-center gap-2">
            <Code className="w-4 h-4 text-brand-accent-soft" />
            <h2 className="text-sm font-mono font-bold text-brand-ink uppercase tracking-wider">
              {pt ? 'Consola de Integração de APIs e Webhooks' : 'API & Webhook Integration Console'}
            </h2>
          </div>
          <button 
            onClick={onClose}
        ...
        {/* Tab switcher */}
        <div className="flex bg-[#070A14] border-b border-brand-border/40 px-4 py-2 gap-2 flex-shrink-0 overflow-x-auto">
          <button
            onClick={() => handleTabChange('erp')}
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition border ${activeApi === 'erp' ? 'bg-brand-accent/15 border-brand-accent text-brand-accent-soft' : 'bg-transparent border-transparent text-brand-ink-dim hover:text-brand-ink'}`}
          >
            <Database className="w-3.5 h-3.5" />
            {pt ? 'REST ERP' : 'REST ERP'}
          </button>

          <button
            onClick={() => handleTabChange('graphql')}
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition border ${activeApi === 'graphql' ? 'bg-brand-accent/15 border-brand-accent text-brand-accent-soft' : 'bg-transparent border-transparent text-brand-ink-dim hover:text-brand-ink'}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition border ${activeApi === 'stripe' ? 'bg-brand-accent/15 border-brand-accent text-brand-accent-soft' : 'bg-transparent border-transparent text-brand-ink-dim hover:text-brand-ink'}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition border ${activeApi === 'mcp' ? 'bg-brand-accent/15 border-brand-accent text-brand-accent-soft' : 'bg-transparent border-transparent text-brand-ink-dim hover:text-brand-ink'}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition border ${activeApi === 'ai' ? 'bg-brand-accent/15 border-brand-accent text-brand-accent-soft' : 'bg-transparent border-transparent text-brand-ink-dim hover:text-brand-ink'}`}
          >
        ...
        {/* Workspace Body */}
        <div className="flex-1 overflow-hidden p-6 flex flex-col gap-6 bg-[#04060C] text-left">
          
          {/* Top Desc */}
          <div className="border-b border-brand-border/40 pb-4 flex-shrink-0">
            <p className="text-xs text-brand-ink leading-relaxed">
              <strong>{pt ? 'Descrição da Integração:' : 'Integration Scope:'}</strong> {pt ? currentScenario.ptDesc : currentScenario.enDesc}
            </p>
          </div>

          {/* Interactive Split Workstation */}
          <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-6 overflow-hidden items-stretch">
            
            {/* Left Column: Request Builder */}
            <div className="bg-[#05070D] border border-brand-border/60 rounded-xl p-4 flex flex-col justify-between overflow-hidden shadow-inner animate-fade-in">
              ...
              <div className="flex flex-col gap-3 flex-1 overflow-hidden">
                ...
                <div className="bg-[#0A0D1A] border border-brand-border rounded px-3 py-2 text-[10px] font-mono text-brand-ink flex items-center gap-2">
                  <span className="text-[#00F2FE] font-bold">{currentScenario.method}</span>
                  <span className="text-brand-ink-dim select-all overflow-x-auto whitespace-nowrap">{currentScenario.url}</span>
                </div>
              ...
              <div className="mt-4 pt-3 border-t border-brand-border/30">
                <button
                  ...
                  disabled={isCalling}
                  className="w-full bg-brand-accent hover:bg-brand-accent-dark text-white font-bold py-3.5 px-6 rounded-xl text-xs transition duration-150 shadow-md shadow-brand-accent/20 flex items-center justify-center gap-2 uppercase tracking-wide disabled:opacity-50"
                >
                  {isCalling ? (
                    <>
            ...
            <div className="bg-[#05070D] border border-brand-border/60 rounded-xl p-4 flex flex-col justify-between overflow-hidden shadow-inner">
              ...
              <div className="flex flex-col gap-4 h-full overflow-hidden">
                <div className="flex justify-between items-center text-[10px] font-mono font-bold text-brand-ink-dim uppercase border-b border-brand-border/30 pb-2 flex-shrink-0">
                  ...
                  {showResponse && (
                    <span className="text-brand-ok font-bold">200 OK</span>
                  )}
                </div>
                ...
                <div className="bg-[#070A14] border border-brand-border/40 rounded-lg p-3 h-[90px] flex flex-col gap-1 font-mono text-[9px] text-[#A0AEC0] overflow-y-auto flex-shrink-0">
                  ...
                  {logs.map((log, idx) => (
                    <div key={idx} className={log && (log.includes('[SUCESSO]') || log.includes('[SUCCESS]') || log.includes('[OK]')) ? 'text-brand-ok font-bold' : log && (log.includes('[ALERT]') || log.includes('[ALERTA]') || log.includes('[DISCREPANCY]') || log.includes('[OVERCHARGE]') || log.includes('[REGRA]')) ? 'text-brand-risk' : ''}>
                      {log}
                    </div>
                  ))}
                  ...
                  {!isCalling && logs.length === 0 && (
                    <span className="text-gray-600 italic">{pt ? 'Aguardando envio de pedido no painel esquerdo.' : 'Awaiting request trigger from the left panel.'}</span>
                  )}
                </div>
                ...
                <div className="flex-1 flex flex-col overflow-hidden">
                  ...
                  <div className="flex-1 bg-[#0A0D1A] border border-brand-border rounded-lg p-3 overflow-y-auto text-[10.5px] font-mono text-brand-ink select-all whitespace-pre">
                    {showResponse ? (
                      ...
                      <span className="text-gray-600 italic">{pt ? '// A resposta do servidor será exibida aqui...' : '// Server response payload will appear here...'}</span>
                    )}
                  </div>
                </div>
